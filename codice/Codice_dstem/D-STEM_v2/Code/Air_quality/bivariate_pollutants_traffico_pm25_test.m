...
for l = 1:size(dati_TRAFFICO, 1)
    indici_righe_test = l;
    indici_righe_train = setdiff(indici_totali, indici_righe_test);

    TRAFFICO_lat_train = TRAFFICO(indici_righe_train(1,1:size(dati_TRAFFICO, 1)-1), 1);
    TRAFFICO_long_train = TRAFFICO(indici_righe_train(1,1:size(dati_TRAFFICO, 1)-1), 2);
    %TRAFFICO_alt_train = TRAFFICO{1,1}{:,2}(indici_righe_train(1,1:size(dati_TRAFFICO, 1)-1));
    
    TRAFFICO_lat_test =  TRAFFICO(indici_righe_test,1);
    TRAFFICO_long_test =  TRAFFICO(indici_righe_test,2);
    %TRAFFICO_alt_test =  TRAFFICO{1,1}{:,2}(indici_righe_test);
    ...
    if sum(totali_lat == TRAFFICO_lat_test) > 1 & sum(totali_long == TRAFFICO_long_test) > 1
        PM25_lat = setdiff(PM25_lat, TRAFFICO_lat_test);
        PM25_long = setdiff(PM25_long, TRAFFICO_long_test);        
        righe_da_togliere = PM25{1,1}{:,3} == TRAFFICO_lat_test;
        PM25_alt = PM25_alt(not(righe_da_togliere));
        dati_train_PM25 = dati_train_PM25(not(righe_da_togliere),:);
    end

    % Estrazione dati train e test
    dati_train_TRAFFICO = dati_TRAFFICO(indici_righe_train(1,1:size(dati_TRAFFICO, 1)-1), :);
    dati_test_TRAFFICO = dati_TRAFFICO(indici_righe_test, :);    
    ...
    for i=1:T
        ...
        X(:,2,i) = TRAFFICO_lat_train;
        X(:,3,i) = TRAFFICO_long_train;  
    ...
    ground.coordinates{1} = [TRAFFICO_lat_train, TRAFFICO_long_train];
    ...
    % KRINGING TRAFFICO
    krig_coordinates_TRAFFICO = [TRAFFICO_lat_test, TRAFFICO_long_test];
    
    obj_stem_krig_grid = stem_grid(krig_coordinates_TRAFFICO, 'deg', 'sparse','point');
    ...
    rmse_nox = nanstd(dati_test_TRAFFICO - y_hat_nox,1,2);
    
    r2_nox = 1 - nanvar(dati_test_TRAFFICO - y_hat_nox,1,2)./nanvar(dati_test_TRAFFICO,1,2);
