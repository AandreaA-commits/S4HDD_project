...
partitions = o_data.kmeans_partitioning(k,trials,lambda,1,1); %the output is the dimension of partitions

%Initial values for model parameters are provided
n_basis = o_fda.get_basis_number;

o_par.beta = o_model.get_beta0();
o_par.theta_z = ones(1,n_basis.z)*3.6; %per questo che f-HDGM si tratta di un HDGM multivariato (latent variable = n basis)
