...
dati_meteo_2019raw{2,1} = anno2020raw;
dati_meteo_2019raw{3,1} = anno2021raw;
dati_meteo_2019raw{4,1} = anno2022raw;
dati_meteo_2019raw{5,1} = anno2023raw;
...
dati_staz = readtable('./stazioni_raw/dati_stazioni_meteo1.csv');
            ...
            for station = stazioni'
                ...
                for row = 1:size(station_filtered, 1)
                    if isempty(stazione)
                        ...
                        f = find(dati_staz{:,2} == station);
                        stazione = [stazione [dati_staz{f,23}, dati_staz{f,22}]];
                    end
...
dati_meteo_2019{1,1} = "Radiazione Ultravioletta [Mw/m2]";
dati_meteo_2019{2,1} = "Velocità del vento [m/s]";
dati_meteo_2019{3,1} = "Direzione del vento [-]";
dati_meteo_2019{4,1} = "Temperatura [ºC]";
dati_meteo_2019{5,1} = "Umidità relativa [%]";
dati_meteo_2019{6,1} = "Precisione barometrica [mb]";
dati_meteo_2019{7,1} = "Radiazione solare [W/m2]";
dati_meteo_2019{8,1} = "Precipitazioni [I/m2]";


for i = 1:12
    unique(dati_meteo_2019raw{1,1}{i,1}.MAGNITUD)
end



save ("dati_meteo_2019", "dati_meteo_2019");
