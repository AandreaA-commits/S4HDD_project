            ...
            for station = stazioni'
                ...
                for row = 1:size(station_filtered, 1)
                    if isempty(stazione)
                        ...
                        stazione = [stazione [dati_staz{f,7}, dati_staz{f,23}, dati_staz{f,22}]];
                    end
...
tabella_poll.CODICE_METEO = [80 81 82 83 86 87 88 89]';
tabella_poll.NOME = {'RADIACIÓN ULTRAVIOLETA', 'VELOCIDAD VIENTO', 'DIR. DE VIENTO ', 'TEMPERATURA', 'HUMEDAD RELATIVA', 'PRESION BARIOMETRICA', 'RADIACION SOLAR', 'PRECIPITACIÓN'}';


%sostituzione del codice della stazione con lalatitudine e longitudine

for i=1:size(dati_meteo_2019,1)
    indice = find(tabella_poll.CODICE_METEO == dati_meteo_2019{i,1});
